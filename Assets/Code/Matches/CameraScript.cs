...
namespace Matches {
    public class CameraScript : MonoBehaviour {
        private readonly static Plane TILE_DETECT_PLANE = new Plane(Vector3.up, new Vector3(0, Tile.LIFT_DIST, 0));
        private readonly static Plane PIECE_DETECT_PLANE = new Plane(Vector3.up, new Vector3(0, UX.Piece.LIFT_DIST, 0));
        ...
        public void Init(int localPlayerIdx, CanvasScript canv, float[][] bounds) {
            ...
            int mask = 1 << (UX.Piece.LAYER + localPlayerIdx);
            mMaskForPieces = mask;
            mMaskForTiles = 1 << (Tile.LAYER + localPlayerIdx);
            for (int i = 0; i < UX.Piece.LAYER; i++) { mask |= (1 << i); }
            mask |= (1 << (Tile.LAYER + localPlayerIdx));
